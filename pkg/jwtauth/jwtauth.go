		...
		authConcrete: &authConcrete{
			...
			Claims: &AuthMapClaims{
				...
				RegisteredClaims: jwt.RegisteredClaims{
					...
					Subject:   "api-key",
					Audience:  []string{"jwt.com"},
					ExpiresAt: jwtTimeDurationCal(31536000), // 1 year in seconds
					NotBefore: jwt.NewNumericDate(now()),
					IssuedAt:  jwt.NewNumericDate(now()),
				},
			},
...
// apiKeyInstant is a global variable that holds the current time in the Asia/Bangkok timezone.
